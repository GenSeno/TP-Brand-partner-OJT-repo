...
trait LogsActivity
{
    ...
    public function logNote(string $content): void
    {
        activity()
            ->performedOn($this)
            ->causedBy(auth()->user())
    ...
    public function logStatus(string $new, string $previous, string $description = 'status-update'): void
    {
        activity()
            ->performedOn($this)
            ->causedBy(auth()->user())
            ->withProperties(['new' => $new, 'previous' => $previous])
            ->log($description);
    }
}
