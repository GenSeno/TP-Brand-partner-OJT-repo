...
class ShippingBreakdown implements CastsAttributes, SerializesCastableAttributes
{
     ...
     * @return \App\Lunar\ValueObjects\Cart\ShippingBreakdown
     */
    public function get($model, $key, $value, $attributes)
    {
        $breakdown = new \App\Lunar\ValueObjects\Cart\ShippingBreakdown;
     ...
     * @param  \Illuminate\Database\Eloquent\Model  $model
     * @param  string  $key
     * @param  \App\Lunar\ValueObjects\Cart\ShippingBreakdown  $value
     * @param  array  $attributes
     * @return array
     */
    public function set($model, $key, $value, $attributes)
    {
        if ($value && !is_a($value, \App\Lunar\ValueObjects\Cart\ShippingBreakdown::class)) {
            throw new \Exception('Shipping breakdown must be instance of App\Lunar\ValueObjects\Cart\ShippingBreakdown');
        }
    ...
    public function serialize($model, $key, $value, $attributes)
    {
        ...
        return $value->items->map(fn($item) => [
            'name' => $item->name,
            'identifier' => $item->identifier,
            'value' => $item->price->value,
            'formatted' => $item->price->formatted,
