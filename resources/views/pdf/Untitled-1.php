                     ...
                     <td style="padding:20px;">
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" border="0">
                           <tr>
                              ...
                              <td width="50%" valign="top" align="right">
                                 ...
                                 <p style="margin:5px 0 0; color:#6c757d; font-size:14px;">PA-{{ str_pad($payment->id, 4, '0', STR_PAD_LEFT) }}</p>
                              </td>
                           </tr>
                        </table>
                        ...
                        <div style="margin-bottom:20px;">
                           ...
                           <div style="font-size:12px; line-height:1.5;">
                              @if($invoice->billingAddress->company_name)
                                  <strong style="font-size:14px;">{{ $invoice->billingAddress->company_name }}</strong><br>
                              @endif
                              {{ $invoice->billingAddress->full_name }}<br>
                              @if($invoice->billingAddress->line1)
                              {{ $invoice->billingAddress->line1 }}
                              @if($invoice->billingAddress->line2) {{, $invoice->billingAddress->line2 }}@endif<br>
                              @endif
                              @if($invoice->billingAddress->barangay || $invoice->billingAddress->city)
                              {{ $invoice->billingAddress->barangay }}, {{ $invoice->billingAddress->city }}@if($invoice->billingAddress->province), {{ $invoice->billingAddress->province }}@endif
                              @if($invoice->billingAddress->country), {{ $invoice->billingAddress->country->name }}@endif<br>
                              @endif
                        ...
                        <table width="100%" cellpadding="8" cellspacing="0" border="1"
                           ...
                           <tr>
                              ...
                              <td>{{ $payment->internal_reference ?? '—' }}</td>
                           </tr>
                           <tr>
