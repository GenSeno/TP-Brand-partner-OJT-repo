    ...
    <form @submit.prevent="submitForm">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-4">
            ...
            <VueDatePicker
              ...
              :time-config="{ enableTimePicker: false }"
              auto-apply
              @update:model-value="form.clearErrors('date_received')"
            />
          ...
          <table class="table table-sm table-bordered">
            ...
            <tbody>
              ...
              <tr v-for="(line, idx) in form.data.lines" :key="line.id || line.temp_id">
                ...
                <td><input-text v-model.number="line.quantity" disabled/></td>
                <td><input-text v-model.number="line.qty_received" /></td>
                <td>
                    <input-text v-if="line.qty_received > 1"
      ...
      <div class="page-footer-buttons">
        <div>
          ...
          <submit-btn :loading="form.processing">Received Purchase Order</submit-btn>
        </div>
      </div>
    </form>
  </Modal>
...
const lineReceivedTotal = (line) => {
    return formatMoney(
        (line.unit_price || 0) * (line.qty_received || 0)
    )
}
...
const submitForm = () => {
  ...
  form.post(route('admin.purchase-order.received', props.purchaseOrder.id), {
    onSuccess: ({ data }) => {
      ...
      alert.showSuccess(data.message || 'Purchase Order received successfully.');
    },
  });
};
