    ...
    <form @submit.prevent="submitForm">
      <div class="page-body new-employee-field">
        <div class="row g-3">
          ...
          <div class="col-md-4">
            ...
            <VueDatePicker
              ...
              :time-config="{ enableTimePicker: false }"
              auto-apply
              @update:model-value="form.clearErrors('expected_delivery_date')"
            />
        ...
        <div class="modal-body-table mt-3">
          <table class="table table-sm table-bordered">
            ...
            <tbody>
              <!-- Existing Lines -->
              <tr v-for="(line, idx) in form.data.lines" :key="line.id || line.temp_id">
                <td>
                  <vue-select
                    ...
                    :append-to-body="true"
                    @input="val => updateDescription(idx, val)"
                  />
                </td>
                <td><input-text v-model.number="line.unit_price" /></td>
                <td><input-text v-model.number="line.quantity" /></td>
                <td>
                    <input-text
              ...
              <tr>
                ...
                <td>
                  <input-text v-model.number="newLine.unit_price" placeholder="Unit Price" />
                </td>
                <td>
                  <input-text v-model.number="newLine.quantity" placeholder="Quantity" />
                </td>
                <td>
                  <input-text
                ...
                <td class="text-center">
                  <button
                    ...
                    @click="addLine"
                    :disabled="!newLine.item || !newLine.unit_price || !newLine.quantity"
                    title="Add line"
                  >
          ...
          <small class="text-info mt-3">
            Click the <span class="text-primary fw-bold">+</span> button to save the row after filling out the form.
          </small>
      ...
      <div class="page-footer-buttons">
        <div>
          ...
          <submit-btn :loading="form.processing">Update Purchase Order</submit-btn>
        </div>
      </div>
    </form>
  </Modal>
...
const form = useAxiosForm({
  ...
  lines: props.purchaseOrder.lines.map(line => reactive({
    ...line,
...
const addLine = () => {
  ...
  const selectedItem = props.inventory.find(item => item.value === newLine.item);
...
const updateDescription = (idx, inventoryId) => {
  const selectedItem = props.inventory.find(item => item.id === inventoryId);
...
const submitForm = () => {
  ...
  form.patch(route('admin.purchase-order.update', props.purchaseOrder.id), {
    onSuccess: ({ data }) => {
      ...
      alert.showSuccess(data.message || 'Purchase Order updated successfully.');
    },
    onError: (errors) => {
      alert.showError(
        errors.message || 'Unable to update Purchase Order due to an error.'
      );
    },
  });
};
