...
const submitFilters = () => {
    filterForm
        ...
        .get(route('admin.order.item.create', props.order.id), {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
...
// FETCH SIZES BASED ON PRINTING OPTIONS SELECTED
watch(selected, () => {
...
const submitForm = () => {
    ...
    form.post(route('admin.order.item.store', props.order.id), {
        data: payload,
        onSuccess: ({ data }) => {
