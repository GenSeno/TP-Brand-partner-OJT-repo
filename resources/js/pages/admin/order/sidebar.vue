        ...
        <div class="card-body">
            ...
            <div class="row relative">
                <div class="col-12">
                    ...
                    <div v-else-if="orders.length === 0" class="alert bg-light">
                        No orders found.
                    </div>
...
const fetchOrders = async () => {
    ...
    try {
        const response = await axios.get(
            route('admin.order.status', { status: selected.value }),
        );
...
const selectOrder = (item) => {
    router.get(route('admin.order.show', { order: item.id }));
};
...
onBeforeUnmount(() => {
    emitter.off('order:updated');
    emitter.off('order:address-updated');
