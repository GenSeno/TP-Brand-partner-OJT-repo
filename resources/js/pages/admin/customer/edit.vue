                ...
                <div class="page-body new-employee-field">
                    ...
                    <div class="mb-3">
                        ...
                        <avatar-upload
                            ...
                            v-model:logo-removed="form.data.avatar_removed"
                            :default-image="
                                form.data.avatar_url || defaultAvatar
                            "
...
const defaultAvatar = '/img/default-edit.png';
const differentShipping = ref(
    props.customer?.shipping && Object.keys(props.customer.shipping).length > 0,
);
const states = ref({
    billing: [],
    shipping: [],
...
const form = useAxiosForm({
    ...
    avatar: null,
    avatar_url: props.customer?.avatar_url || null,

    email: props.customer.billing[0]?.email || '',
    phone:
        props.customer.billing[0]?.phone ||
        props.countries.default?.phone ||
        '+63',
    line1: props.customer.billing[0]?.line1 || '',
    line2: props.customer.billing[0]?.line2 || '',
    province: props.customer.billing[0]?.province || '',
    city: props.customer.billing[0]?.city || '',
    barangay: props.customer.billing[0]?.barangay || '',
    postcode: props.customer.billing[0]?.postcode || '',
    country_id:
        props.customer.billing[0]?.country_id ||
        props.countries.default?.id ||
        '',

    shipping: {
        title: props.customer.shipping[0]?.title || '',
        first_name: props.customer.shipping[0]?.first_name || '',
        last_name: props.customer.shipping[0]?.last_name || '',
        line1: props.customer.shipping[0]?.line1 || '',
        line2: props.customer.shipping[0]?.line2 || '',
        province: props.customer.shipping[0]?.province || '',
        city: props.customer.shipping[0]?.city || '',
        barangay: props.customer.shipping[0]?.barangay || '',
        postcode: props.customer.shipping[0]?.postcode || '',
        country_id:
            props.customer.shipping[0]?.country_id ||
            props.countries.default?.id ||
            '',
    },
});
...
const submitForm = () => {
    ...
    formData.append('avatar_removed', form.data.avatar_removed ? '1' : '0');
    ...
    form.submit('post', route('admin.customer.update', props.customer.id), {
        data: formData,
        headers: { 'Content-Type': 'multipart/form-data' },
        onSuccess: (data) => {
            alert.showSuccess(data.message || 'Customer updated successfully.');
            modalRef.value?.close(); // should close
            emitter.emit('customer:updated', data.customer || null);
        },
    });
};
