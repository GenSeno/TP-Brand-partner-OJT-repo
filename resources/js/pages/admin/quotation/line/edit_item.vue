    ...
    <div>
        ...
        >
            <div class="page-header">
                <h4>Edit Product Items - {{ props.product?.name }}</h4>
            </div>
...
// Category & Product
const selectedCategoryId = ref(props.product.category_id);
...
const openSetNamesOverlay = (variant) => {
    ...
    const qty = variantQuantities[variant.quote_line_id] || 1;
    tempNames.value = [
        ...(variantNames[variant.quote_line_id] || Array(qty).fill('')),
    ];
...
const saveNames = () => {
    if (currentVariantForNames.value) {
        variantNames[currentVariantForNames.value.quote_line_id] = [
            ...tempNames.value,
        ];
...
// Submit form
const form = useAxiosForm({ quote_lines: props.lines });
...
const submitForm = () => {
    ...
    form.submit(
        'post',
        route('admin.quotation.lines.update_bulk', {
        ...
        }),
        {
            data: { quote_lines },
            onSuccess: (res) => {
                alert.showSuccess(res.message || 'Items updated successfully.');
                modalRef.value?.close();
                emitter.emit('quotationline:updated', res.quote_lines);
            },
        },
    );
};
