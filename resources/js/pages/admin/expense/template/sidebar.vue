<template>
    <div class="card">
        <div class="card-body">
            <!-- Dropdown filter -->
            <div class="relative inline-block w-56 mb-2">
                <div class="dropdown">
                    Payment Voucher List
                    <button
                        class="w-full border rounded bg-white float-end"
                        type="button"
                        data-bs-toggle="dropdown"
                        data-bs-auto-close="outside"
                        aria-expanded="false"
                    >
                        <i
                            data-feather="chevron-down"
                            class="feather-chevron-down mt-1"
                        ></i>
                    </button>

                    <ul class="dropdown-menu dropdown-menu-end form-control">
                        <li>
                            <label
                                v-for="status in statuses"
                                :key="status.value"
                                class="d-block py-1 cursor-pointer"
                            >
                                <input
                                    type="checkbox"
                                    :value="status.value"
                                    v-model="selected"
                                    @change="fetchExpenses"
                                />
                                {{ status.label }}
                            </label>
                        </li>
                        <hr />
                        <li>
                            <div class="d-flex justify-content-end gap-2 px-2">
                                <button
                                    type="button"
                                    class="btn btn-xs btn-primary"
                                    @click.stop="selectAllAndFetch"
                                >
                                    Select all
                                </button>
                                <button
                                    type="button"
                                    class="btn btn-xs btn-secondary"
                                    @click.stop="clearAllAndFetch"
                                >
                                    Clear
                                </button>
                            </div>
                        </li>
                    </ul>
                </div>
            </div>

            <hr />

            <!-- Quotation List -->
            <div class="row relative">
                <div class="col-12">
                    <!-- Loading / Empty states -->
                    <div v-if="loading1" class="d-flex justify-content-center">
                        <i class="spinner-border spinner-border-sm me-1"></i
                        >Loading...
                    </div>
                    <div
                        v-else-if="expenses.length === 0"
                        class="alert bg-light"
                    >
                        No payment voucher found.
                    </div>

                    <!-- Quotation Items -->
                    <div v-else class="list-group">
                        <div
                            v-for="exp in paginatedExpense"
                            :key="exp.id"
                            class="list-group-item d-flex flex-column justify-content-center cursor-pointer"
                            :class="
                                exp.id === expense.id
                                    ? 'border-start-primary'
                                    : 'border-transparent'
                            "
                            @click="selectExpense(exp)"
                        >
                            <a href="#">
                                <div class="row">
                                    <label class="col-md-8 mb-0 fw-bold fs-12">
                                        {{ exp.supplier.name }}
                                    </label>
                                    <label
                                        class="col-md-4 mb-0 fw-bold fs-10 text-end text-muted"
                                    >
                                        {{ exp.total_amount.formatted }}
                                    </label>
                                </div>
                                <div class="row mt-1">
                                    <div class="col-md-6">
                                        <small class="fs-10">{{
                                            exp.reference
                                        }}</small>
                                    </div>
                                    <div class="col-md-6 text-end text-muted">
                                        <small class="fs-10">
                                            <i
                                                data-feather="calendar"
                                                class="feather-calendar mt-1"
                                            ></i>
                                            {{
                                                dayjs(exp.expense_date).format(
                                                    dateFormat,
                                                )
                                            }}
                                        </small>
                                    </div>
                                </div>
                                <div class="row mt-1">
                                    <div class="col-md-12">
                                        <small class="fs-10 text-capitalize"
                                            ><span
                                                :class="statusText(exp.status)"
                                                >{{ exp.status }}</span
                                            ></small
                                        >
                                    </div>
                                </div>
                            </a>
                        </div>
                    </div>

                    <!-- Pagination Controls -->
                    <nav class="mt-2">
                        <ul class="pagination justify-content-center gap-2">
                            <li
                                class="page-item"
                                :class="{ disabled: currentPage === 1 }"
                            >
                                <a
                                    class="page-link btn btn-xs"
                                    href="#"
                                    @click.prevent="prevPage"
                                    >Prev</a
                                >
                            </li>
                            <li
                                class="page-item"
                                :class="{
                                    disabled: currentPage === totalPages,
                                }"
                            >
                                <a
                                    class="page-link btn btn-xs"
                                    href="#"
                                    @click.prevent="nextPage"
                                    >Next</a
                                >
                            </li>
                        </ul>
                    </nav>
                </div>
            </div>
        </div>
    </div>
</template>

<style scoped>
.border-start-primary {
    border-left: 5px solid #fe9f43;
}
</style>

<script setup>
import { ref, computed, onMounted, onBeforeUnmount } from 'vue';
import { router } from '@inertiajs/vue3';
import axios from 'axios';
import dayjs from 'dayjs';
import { statusText } from '@/helpers/status';
import { emitter } from '@/composables/eventBus';

const loading1 = ref(false);
const expenses = ref([]);
const selected = ref(['draft', 'upcoming','paid','cancelled']);
const currentPage = ref(1);
const perPage = ref(20); // show 5 items per page

const props = defineProps({
    expense: Object,
    template: String,
});
const dateFormat = ref('MMM. DD, YYYY');

const statuses = [
    { label: 'Draft', value: 'draft' },
    { label: 'Upcoming', value: 'upcoming' },
    { label: 'Paid', value: 'paid' },
    { label: 'Cancelled', value: 'cancelled' },
];

const updateExpenseEmitterEvent = ref('expense:updated');

const selectAllAndFetch = () => {
    selected.value = statuses.map((s) => s.value); // select all
    fetchExpenses(); // fetch after select all
};

const clearAllAndFetch = () => {
    selected.value = []; // clear selection
    fetchExpenses(); // fetch after clear
};

// computed pagination
const totalPages = computed(() =>
    Math.ceil(expenses.value.length / perPage.value),
);
const paginatedExpense = computed(() => {
    const start = (currentPage.value - 1) * perPage.value;
    return expenses.value.slice(start, start + perPage.value);
});

const fetchExpenses = async () => {
    loading1.value = true;
    try {
        const response = await axios.get(
            route('admin.expense.status', { status: selected.value }),
        );
        expenses.value = response.data;
        currentPage.value = 1; // reset page on new data
    } catch (error) {
        console.error('Error fetching expense:', error);
    } finally {
        loading1.value = false;
    }
};

// select expense
const selectExpense = (expense) => {
    if (props.template === 'template') {
        router.get(route('admin.expense.show', { expense: expense?.id }));
    } else {
        router.get(route('admin.expense.item', { expense: expense?.id }));
    }
};

// pagination methods
const prevPage = () => {
    if (currentPage.value > 1) currentPage.value--;
};
const nextPage = () => {
    if (currentPage.value < totalPages.value) currentPage.value++;
};

/* Close dropdown when clicking outside */
const open = ref(false);
const handleClickOutside = (e) => {
    if (!e.target.closest('.relative')) open.value = false;
};

onMounted(() => {
    fetchExpenses();
    emitter.on(updateExpenseEmitterEvent.value, () => {
        fetchExpenses(); // refresh sidebar
    });
    // emitter.on('adjustment:updated', () => {
    //   fetchExpenses(); // refresh sidebar
    // });
    // emitter.on('quotationline:created', () => {
    //   fetchExpenses(); // refresh sidebar
    // });
    // emitter.on('quotationline:updated', () => {
    //   fetchExpenses(); // refresh sidebar
    // });
    // emitter.on('quote_lines:deleted', () => {
    //   fetchExpenses(); // refresh sidebar
    // });
    document.addEventListener('click', handleClickOutside);
});

onBeforeUnmount(() => {
    emitter.off(updateExpenseEmitterEvent.value);
    //  emitter.off('adjustment:updated');
    //  emitter.off('quotationline:created');
    //  emitter.off('quotationline:updated');
    document.removeEventListener('click', handleClickOutside);
});

defineExpose({
    fetchExpenses,
});
</script>
