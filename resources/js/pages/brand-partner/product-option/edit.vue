...
const form = useAxiosForm({
    ...
    values: props.option?.values?.map((v) => ({ label: v.label, value: v.value ?? '' })) || [{ label: '', value: '' }],
});
...
const submitForm = () => {
    form.put(route('brand-partner.product-options.update', props.option.id), {
        onSuccess: ({ data }) => {
            modalRef.value.close();
            alert.showSuccess(data.message || 'Product option updated successfully.');
        },
    });
};
</script>
